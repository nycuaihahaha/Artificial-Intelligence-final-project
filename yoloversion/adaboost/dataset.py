...
import numpy as np
import adaboost
clf = adaboost.Adaboost.load('final_adaboost/clf_300_1_10')
...
def load_and_save(image_name,x,y,w,h):
    for i in os.walk('final_adaboost/result/'):
        if image_name in i[2]:
            path = 'final_adaboost/result/'
            break
        else:
            path = 'final_adaboost/mask/images/'
    ...
    if clf.classify(cv2.resize(img, (36, 16))):
        imgo = cv2.resize(imgo, (416, 416))
        cv2.rectangle(imgo, (max(int(x) - int(w / 2), 0), max(int(y) - int(h / 2), 0)), (int(x) + int(w / 2), int(y)+int(h/ 2)), (0, 255, 0), 2)
    else:
        imgo = cv2.resize(imgo, (416, 416))
        cv2.rectangle(imgo, (max(int(x) - int(w / 2), 0), max(int(y) - int(h / 2), 0)), (int(x) + int(w / 2), int(y)+int(h/ 2)), (0, 0, 255), 2)
    #cv2.imshow("mask", imgo)
    #cv2.waitKey(0)
    cv2.imwrite('final_adaboost/result/'+image_name, cv2.resize(imgo,(wo,ho)) )
